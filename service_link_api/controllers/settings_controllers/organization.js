...
export async function getAllOrganizations(req, res){
    try{
        ...
        res.status(201).json({organizations , message: `Organization data fetched successfully!`});
    }catch(error){
        res.status(500).json({ message: `Encountered an internal error ${error}` });
    }
}
...
export async function updateOrganization(req, res){
    try{
        ...
        if (updatedRows === 0) {
            return res.status(404).json({ message: `No organization found with id ${req.params.id}` });
        }
...
export async function archiveOrganizationById(req, res){
    try{
        ...
        // If no rows were updated, it means the reference number didn't match any requisition
        if (updatedRows === 0) {
            return res.status(404).json({ message: `No organization found with reference number ${req.params.id}` });
        }

        res.status(200).json({ message: req.params.archive === '0' ? 'Organization removed from archive!' : 'Organization added to archive!'});
    }catch(error){
