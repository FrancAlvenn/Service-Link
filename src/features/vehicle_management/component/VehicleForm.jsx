...
const VehicleForm = ({ mode = "add", initialValues, onClose, onSuccess }) => {
  const { user } = useContext(AuthContext);
  const { fetchVehicles, createVehicleUnavailability, fetchVehicleUnavailability } =
  ...
  // Fetch existing unavailability if editing
    useEffect(() => {
      if (mode === "edit" && initialValues?.vehicle_id) {
        fetchVehicleUnavailability(initialValues.vehicle_id).then((data) => {
  ...
  const handleNext = () => {
    if (activeStep === 0) {
      // Validate first step
      if (!vehicle.name || !vehicle.vehicle_type) {
  ...
  const submitVehicle = async () => {
    try {
      const endpoint =
        mode === "edit"
          ? `${process.env.REACT_APP_API_URL}/vehicles/${vehicle.reference_number}`
      ...
      const response = await axios[method](
        endpoint,
        { ...vehicle, user: user?.reference_number || "system" },
        { withCredentials: true }
      );

      if ([200, 201].includes(response.status)) {
        let vehicleId = initialValues?.vehicle_id;
        
        // For new vehicles, get the ID from the response or fetch it
        if (mode === "add" && response.data?.newVehicle) {
          vehicleId = response.data.newVehicle.vehicle_id;
        } else if (mode === "add" && !vehicleId) {
          // If ID not in response, we might need to fetch by reference number
          // For now, we'll try to get it from the response or skip unavailability creation
          console.warn("Vehicle ID not found in response, skipping unavailability creation");
        }

        // Save unavailability periods
        if (unavailabilityEvents.length > 0 && vehicleId) {
        ...
        ToastNotification.success(
          "Success!",
          `Vehicle ${mode === "edit" ? "updated" : "created"} successfully.`
        );
        fetchVehicles();
  ...
  const CustomCalendar = () => {
    ...
    const DayView = () => {
      ...
      // Generate time slots (6 AM to 11 PM for vehicles - longer hours than vehicles)
      const timeSlots = [];
    ...
    <div className="h-full bg-white rounded-lg w-full px-3 flex flex-col">
      <div className="py-4 px-5 mb-5 shadow-sm">
        ...
        <Typography variant="small" className="mb-2">
          {mode === "edit"
            ? "Update vehicle information and unavailability schedule."
            : "Enter vehicle details and set unavailability schedule."}
        </Typography>
      </div>
      ...
      <div className="mb-4 px-5 py-2 bg-gray-50 rounded-lg border border-gray-200">
        <div className="flex items-center justify-between max-w-2xl mx-auto">
          ...
          <div className="flex flex-col items-center flex-1 relative">
            <button
              onClick={() => {
                if (activeStep > 0 || vehicle.name) {
                  setActiveStep(1);
                }
              }}
              disabled={activeStep === 0 && !vehicle.name}
              className={`flex items-center justify-center w-8 h-8 rounded-full border-2 transition-all duration-300 ${
                activeStep === 1
                  ? "bg-blue-600 border-blue-600 text-white"
                  : activeStep > 1
                  ? "bg-green-500 border-green-500 text-white"
                  : activeStep === 0 && !vehicle.name
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    ...
                    value={vehicle.booking_advance_days || 7}
                    onChange={handleChange}
                    className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm"
                    min="1"
                  />
                ...
                <div className="flex items-center mt-6">
                  <input
                    ...
                    checked={vehicle.requires_approval || false}
                    onChange={handleChange}
                    className="mr-2"
                  />
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    ...
                    value={vehicle.assigned_department || ""}
                    onChange={handleChange}
                    className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm"
                  />
                </div>
        ...
        {activeStep === 1 && (
          <div className="space-y-4">
            ...
            <Typography variant="small" className="mb-4 text-gray-600">
              Select dates on the calendar to mark periods when this vehicle is
