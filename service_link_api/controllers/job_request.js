...
export async function createJobRequestDetails(req, res) {
    let transaction;
    try{
        transaction = await sequelize.transaction();

        const jobRequestDetailsData = req.body.jobRequestDetails.map(detail => ({
          job_request_id: req.params.job_request_id,
          qty: detail.qty || null,
          particulars: detail.particulars,
          description: detail.description,
          remarks: detail.remarks || null,
        ...
        const createdJobRequestDetails = await JobRequestDetails.bulkCreate(jobRequestDetailsData);

        await transaction.commit();

        res.status(201).json(createdJobRequestDetails);
    }catch (error){
...
export async function updateJobRequestDetails(req, res) {
    let transaction;
    try{
        transaction = await sequelize.transaction();

        const [updatedRows] = await JobRequestDetails.update({
          qty : req.body.qty,
        ...
        createLog({
            ...
            performed_by: req.body.requester_id,
            target: req.params.reference_number,
            details: `Job Requisition ${req.params.reference_number} updated successfully!`,
        });
    }catch (error){
        res.status(500).json({ message: `Encountered an internal error ${error}`})
        transaction.rollback();
