...
export async function getAllDesignation(req, res){
    try{
        ...
        res.status(201).json({designations , message: `Designation data fetched successfully!`});
    }catch(error){
        res.status(500).json({ message: `Encountered an internal error ${error}` });
    }
}
...
export async function getDesignationById(req, res){
    try{
        ...
        res.status(201).json({designation , message: `Designation data fetched successfully!`});
    }catch(error){
        res.status(500).json({ message: `Encountered an internal error ${error}` });
    }
}
...
export async function updateDesignation(req, res){
    try{
        ...
        if (updatedRows === 0) {
            return res.status(404).json({ message: `No  n found with the reference number: ${req.params.id}` });
        }
...
export async function archiveDesignationById(req, res){
    try{
        ...
        // If no rows were updated, it means the reference number didn't match any requisition
        if (updatedRows === 0) {
            return res.status(404).json({ message: `No requisition found with reference number ${req.params.id}` });
        }
