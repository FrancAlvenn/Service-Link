...
import { ArrowLeft, DeviceMobileCamera, Envelope, Lock, Tag, User } from "@phosphor-icons/react";
...
function AccountSetting({ onClose }) {
    ...
    useEffect(() => {
        ...
        if (user?.reference_number) {
            setCurrentUser(allUserInfo.find((_user) => _user.reference_number === user?.reference_number));
        }
    }, []);
    ...
    const handleSaveField = async (key) => {
        try {
            if (key === "password") {
                ...
                if (key === "contact_number" && !phoneRegex.test(currentUser.contact_number)) {
                    setCurrentUser((prev) => ({ ...prev, contact_number: "" }));
                    ToastNotification.info("Notice!", "Please enter a valid phone number.");
                    return;
                }

                const response = await axios.put(`/users/${currentUser.reference_number}`, {
                    [key]: currentUser[key],
                });
    ...
    return (
        <div className="fixed top-0 right-0 w-full max-w-[750px] h-full bg-white dark:bg-gray-900 text-gray-900 dark:text-white z-100 p-6 overflow-y-auto">
            <div className="flex flex-col justify-between items-start mb-6 w-full">
                ...
                <Typography variant="h5" className="font-bold text-xl mb-4 dark:text-white">
                    Account Settings
                </Typography>

                <div className="flex flex-col gap-3 pt-5 pb-2 px-5 border border-gray-200 dark:border-gray-700 rounded-lg w-full">
                    <Typography className="text-sm font-semibold dark:text-gray-300">Personal Information</Typography>

                    {[
                        { label: "First Name", icon: <User size={20} />, key: "first_name", editable: true },
                        { label: "Last Name", icon: <User size={20} />, key: "last_name", editable: true },
                        { label: "Email", icon: <Envelope size={20} />, key: "email", editable: false },
                        { label: "Phone Number", icon: <DeviceMobileCamera size={20} />, key: "contact_number", editable: true },
                        { label: "Username", icon: <Tag size={20} />, key: "username", editable: true },
                    ].map(({ label, icon, key, editable }) => (
                        <div
                            key={key}
                        ...
                        >
                            ...
                            <div className="flex flex-col gap-0 w-full">
                                <Typography className="text-xs font-medium text-gray-500 dark:text-gray-400">{label}</Typography>
                                {editable && editingField === key ? (
                                    ...
                                    <Typography className="text-sm font-medium py-0 dark:text-gray-200">{currentUser[key]}</Typography>
                                )}
                            </div>
                        </div>
                    ))}
                    ...
                    <div className="flex gap-2 py-4 px-2 items-center hover:bg-gray-100 dark:hover:bg-gray-800 cursor-pointer">
                        <Lock size={20} />
                        <div className="flex flex-col gap-0 w-full">
                            <Typography className="text-xs font-medium text-gray-500 dark:text-gray-400">Password</Typography>
                            {editingField === "password" ? (
                                <>
                                    ...
                                    <input
                                        ...
                                        className="text-sm p-2 w-full border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-black dark:text-white"
                                    />
                                    <Button variant="filled" color="blue" size="sm" className="mt-2 dark:bg-blue-600" onClick={() => handleSaveField("password")}>
                                        Confirm Password Change
                                    </Button>
                                    <Button variant="filled" color="red" size="sm" className="mt-2 dark:bg-red-600" onClick={() => setEditingField(null)}>
                                        Cancel
                                    </Button>
                                </>
                            ) : (
                                <Typography className="text-sm font-medium py-0 text-blue-500 dark:text-blue-400" onClick={() => setEditingField("password")}>
                                    Change Password
                                </Typography>
                            )}
                        </div>
