...
export async function getAllDepartments(req, res){
    try{
        ...
        res.status(201).json({departments , message: `Department data fetched successfully!`});
    }catch(error){
        res.status(500).json({ message: `Encountered an internal error ${error}` });
    }
}
...
export async function updateDepartment(req, res){
    try{
        ...
        if (updatedRows === 0) {
            return res.status(404).json({ message: `No department found with the reference number: ${req.params.id}` });
        }
