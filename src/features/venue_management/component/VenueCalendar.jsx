...
const AdminVenueCalendar = () => {
  const { venues, fetchVenues, fetchAllVenueUnavailability, fetchVenueBookings,fetchVenueUnavailabilityById, loading } = useContext(VenueContext);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (mounted) setError("Failed to load venue data");
      } finally {
    ...
    const loadByFilter = async () => {
      try {
        ...
        if (venueFilter === "all") {
          ...
          const vid = venueFilter;
          const [u, b] = await Promise.all([
            typeof fetchVenueUnavailability === "function" ? fetchVenueUnavailabilityById(vid) : fetchAllVenueUnavailability(),
            fetchVenueBookings(vid),
          ]);
          if (mounted) {
            setUnavailability(u || []);
            setBookings(b || []);
  ...
  const selectedVenueId = venueFilter === "all" ? null : venueFilter;

  const events = useMemo(() => {
  const getVenueId = (item) => {
  ...
  const getVenueName = (vid) => {
    const venue = venues.find((v) => 
      String(v.venue_id || v.id) === String(vid)
    );
    return venue?.name || venue?.reference_number || "Unknown Venue";
  };

  const shouldInclude = (item) => {
    if (venueFilter === "all") return true;
    return String(getVenueId(item)) === String(venueFilter);
  };

  const bookingEvents = (bookings || [])
    .filter(shouldInclude)
    .map((b) => {
      const vid = getVenueId(b);
      const start = b.start_date || b.booking_date || b.start;
      const end = b.end_date || b.end;

      return {
        title: `${getVenueName(vid)} - ${b.event_title || b.title || "Booked"}`,
        start: start,
        end: end || undefined,
  ...
  const unavailEvents = (unavailability || [])
    .filter(shouldInclude)
    .map((u) => {
      const vid = getVenueId(u);
      return {
        title: `${getVenueName(vid)} - ${u.reason || "Unavailable"}`,
...
}, [bookings, unavailability, venues, venueFilter]);

  const statusStyleColor = (status) => {
    switch (status) {
  ...
  const venueColor = (vid) => {
    const palette = ["#06b6d4","#3b82f6","#10b981","#f59e0b","#8b5cf6","#ec4899","#14b8a6","#ef4444","#6366f1","#84cc16"];
    const idx = Math.abs(parseInt(String(vid).replace(/\D/g, ""), 10) || 0) % palette.length;
    return palette[idx];
  };
  ...
  return (
    <div className="w-full p-4 border border-gray-200 rounded-lg bg-white shadow-sm mb-4 min-h-screen flex flex-col" role="region" aria-label="Admin Venue Calendar" aria-live="polite">
      <Card className="shadow-none">
        <CardHeader floated={false} shadow={false} className="rounded-none flex justify-between items-center flex-wrap gap-2">
          <div>
            <Header title="Venue Calendar" description="Consolidated view of venue availability." />
          </div>
        </CardHeader>
            ...
            <div className="w-full flex-1 flex flex-col">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2 items-center justify-end mb-5">
                  <select id="venue-select" aria-label="Venue" data-testid="venue-select" className="px-3 py-2 text-sm bg-white border rounded-md" value={venueFilter} onChange={(e) => setVenueFilter(e.target.value)}>
                    <option value="all">All venues</option>
                    {venues.map((v) => (
                      <option key={v.venue_id || v.id} value={v.venue_id || v.id}>{v.name || v.reference_number || `Venue ${v.venue_id || v.id}`}</option>
                    ))}
                  </select>
              ...
              <div className="h-full">
                <FullCalendar
                  ...
                  eventClick={(info) => {
                    ...
                    const ref = info.event.extendedProps?.venue_request_id;
      ...
      {open && (
        <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }} transition={{ duration: 0.2 }}
          ...
          <div className="flex justify-between items-center mb-3">
            <Typography className="text-base font-semibold">Venue Unavailability</Typography>
            <Button variant="text" size="sm" onClick={onClose} aria-label="Close">Close</Button>
          </div>
...
export default AdminVenueCalendar;
